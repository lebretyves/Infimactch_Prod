...
Version du 14 septembre 2026 — Périmètre validé — Projet Epitech D-WEB-901
...
2. Les décisions explicites validées par Yves dans les échanges, pour le fonctionnement métier et les arbitrages V1.
3. Le catalogue fourni Interimatch_Sante_Catalogue_Complet_Fonctionnalites.docx, avec ses références F01 à F26 et F02 bis.
4. Ce prompt, qui consolide ces sources et remplace les anciens arbitrages incompatibles.
...
Commence par lire la note de reprise associée et les instructions applicables au dépôt. Inspecte l’arborescence, les dépendances, les migrations, les modules et les tests existants. Identifie le commit de départ et les modifications non commitées ; préserve le travail des autres membres.
...
Conserve la stack validée : NestJS, Node.js et TypeScript ; PostgreSQL avec PostGIS ; MongoDB ; n8n ; frontend Next.js/TypeScript développé avec l’équipe. Ne remplace pas un ORM ou une bibliothèque déjà cohérente sans raison démontrée. Vérifie les versions maintenues et compatibles, utilise un lockfile, et documente les choix. N’installe pas des versions flottantes « latest » dans une livraison reproductible.
...
L’historique conserve les transitions, leur auteur et leur date. Une mission pourvue dont l’affectation est annulée ne rouvre pas silencieusement : l’agence décide de sa republication et les consentements sont réévalués. Retirer une candidature après affectation est refusé ; l’annulation suit le parcours autorisé et tracé de mission/affectation, sans créer le module d’imprévus F11.
...
L’affectation est une transaction PostgreSQL : vérifier les droits, verrouiller mission et professionnel dans un ordre stable, relire la version, la candidature active, les prérequis, disponibilités, vérifications et conflits, puis créer l’affectation, passer la mission à FILLED et écrire l’événement d’outbox. En cas d’échec, aucune de ces écritures ne subsiste partiellement.

Ajoute une contrainte d’affectation active unique par mission et une contrainte d’exclusion des chevauchements par professionnel sur les intervalles actifs, avec la migration PostgreSQL adaptée. Une modification de disponibilité ou d’indisponibilité utilise le même verrou du professionnel et ne peut contredire silencieusement une affectation existante. Deux requêtes concurrentes incompatibles produisent un succès et un conflit 409, jamais deux affectations.
...
Utilise HTTPS en transit pour l’environnement de démonstration déployé. Ne publie pas directement PostgreSQL, MongoDB ou l’éditeur n8n sur Internet. Paramètre les requêtes SQL ; construis les filtres MongoDB à partir de DTO validés. Protège les URLs de fournisseurs contre SSRF par une liste d’origines autorisées ; aucun fetch libre d’une URL utilisateur. Échappe ou assainis les contenus selon le contexte d’affichage pour éviter XSS.
...
| Affectation | Deux validations concurrentes ; rollback ; unicité mission ; chevauchement professionnel ; modification concurrente de disponibilité. |
...
Le projet dure 11 jours. Le CDC, la roadmap et le chiffrage par fonctionnalité/module sont attendus à J+2, avec un go/no-go. Estime d’après le dépôt réel, la disponibilité de l’équipe et les dépendances ; les anciennes hypothèses d’heures ne sont pas des temps validés ni des temps réellement passés.
...
| R11 | Chiffrement repos/transit, p. 2 | §§14,16 ; RIB/pièce fictifs, clés et HTTPS. |
...
Passe 1 — Fidélité documentaire. Compare le résultat au sujet ligne par ligne avec R01 à R40, puis au catalogue et aux arbitrages V1. Vérifie F02 bis, les favoris établissements, F15, F18 et F22. Contrôle que les fonctions V2/V3/V4 restent à leur place. Les anciens renvois du catalogue à un identifiant de contrat dépassé sont remplacés par F24 dans la documentation dérivée, sans prétendre avoir modifié le fichier source.
...
Le travail est terminé quand le comportement prévu, les droits, les erreurs, les tests adaptés, la documentation et les preuves sont disponibles, ou quand les blocages restants sont explicitement présentés. Ne promets pas une conformité de production à 100 %, une sécurité absolue ou une note au jury.
...
Ajoute une matrice de conformité et un journal des temps estimés/réels. Ne conserve aucun secret ni donnée personnelle dans ces notes. Ne réécris pas l’historique pour transformer un test échoué en succès : conserve l’échec, la correction et le résultat du rejeu.
