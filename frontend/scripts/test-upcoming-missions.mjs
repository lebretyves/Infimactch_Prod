...
try {
  ...
  for(const width of [1440,768,375]){await page.setViewportSize({width,height:1100});await card.scrollIntoViewIfNeeded();assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await card.screenshot({path:`artifacts/upcoming-missions/upcoming-${width}.png`});await page.screenshot({path:`artifacts/upcoming-missions/home-${width}.png`,fullPage:true});}
  ...
  assert.deepEqual(writes.filter(path=>!path.endsWith('/auth/activity')),[]);assert.deepEqual(errors,[]);console.log('PASS upcoming sort/1preview/status/date/DOM/location/link/empty/enterprise notifications/375+1440');
