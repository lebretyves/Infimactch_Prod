...
try{
 ...
 else if(path.endsWith('/profile/availability')){const body=route.request().postDataJSON();writes.push(body);for(const c of body.changes){profile.available=profile.available.filter(p=>p.start!==c.start);profile.unavailable=profile.unavailable.filter(p=>p.start!==c.start);if(c.state==='available')profile.available.push({start:c.start,end:c.end});if(c.state==='unavailable')profile.unavailable.push({start:c.start,end:c.end});}json={available:profile.available,unavailable:profile.unavailable};}
 ...
 assert.equal(await page.getByRole('combobox',{name:'Ville de référence'}).count(),1);assert.deepEqual(errors,[]);console.log('PASS compact month: 7 columns, 35 day targets, keyboard editing, three-state persistence, confirmed lock, DST night, month/week navigation, mobility retained, no overflow375/768/1440.');
