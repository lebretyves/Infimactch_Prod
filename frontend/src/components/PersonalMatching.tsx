...
export function PersonalMatching({id, userId}: {id: string; userId: string}) {
  const r = useRemote(signal => api<{score: number | null; reasons: string[]}>("/me/matches/mission/" + encodeURIComponent(id.slice(2)), {signal}), userId + ":matching:" + id);
  return <section aria-label="Votre taux de matching">
    ...
    {r.loading ? <p role="status">Calcul du taux…</p> : r.error ? <p role="alert">Le taux est indisponible. <button type="button" onClick={r.reload}>Réessayer</button></p> : r.data && <>
      <p><strong>{r.data.score !== null && Number.isFinite(r.data.score) ? `Taux de matching : ${Math.round(r.data.score)} %` : "Taux de matching non calculable"}</strong></p>
