...
export function MatchingRules() {
  ...
  const r = useRemote(async signal => {
    const data=await api<{weights: Record<"C" | "Z" | "D" | "E", number>; rppsRequired?: boolean}>("/matching/rules", {signal});
  ...
  return <div><details style={{border: "1px solid var(--line)", borderRadius: 12, padding: 16, marginBlock: 16}}>
    ...
    <p>Pour proposer automatiquement une mission, le moteur vérifie le métier, les compétences requises, l’expérience minimale, les disponibilités, l’absence de conflit, les horaires acceptés et le rayon de mobilité. Les horaires doivent être confirmés et la mission ouverte.</p>
    {r.data && <p>{r.data.rppsRequired === false ? "Projet de démonstration : le RPPS est facultatif pour le matching, la candidature et l’affectation des missions internes. Le statut professionnel affiché n’est pas modifié." : "La vérification RPPS est également requise pour le matching, la candidature et l’affectation."}</p>}
