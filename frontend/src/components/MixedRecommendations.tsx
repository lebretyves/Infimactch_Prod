...
export function MixedRecommendations({ userId, isSaved, onFavorite, busy }: { userId: string; isSaved: (item: Listing) => boolean; onFavorite: (item: Listing) => void; busy: boolean }) {
  ...
  const result = useRemote(signal => getRecommendations(signal, origin), userId + ":" + origin);
  const data = result.data;
  function cards(items: Recommendation[], external: boolean) {
    return <ul>{items.map(item => <li className={s.item} key={item.id}>
      <span className={s.source}>{external ? sourceLabel(item) : 'Partenaire InfiMatch'}</span>
      <h4>{item.title}</h4>
      ...
      <div className={s.actions}>
        <ButtonLink to={'/missions/' + item.id + (!external && item.match_explanation_id ? '?correspondance=' + encodeURIComponent(item.match_explanation_id) : '')} size="sm">{external ? 'Voir l’offre externe' : 'Voir la mission partenaire'}</ButtonLink>
  ...
  return <section className={u.card} aria-labelledby="recommendations-heading">
    <div className={u.row}><h2 id="recommendations-heading">Vos pistes de mission</h2><Link to={`/missions?origine=${origin}`}>Toutes les offres →</Link></div>
    <OfferOriginChoices value={origin} onChange={origine => {const next=new URLSearchParams(params);next.set("origine",origine);setParams(next);}} />
    {result.loading ? <p role="status">Recherche de missions compatibles et d’offres externes…</p> : result.error ? <div role="alert" className={s.notice}><p>Les suggestions ne sont pas disponibles pour le moment. Vos autres informations restent accessibles.</p><Button variant="outline" onClick={result.reload}>Réessayer les suggestions</Button></div> : data && <>
      {origin !== 'externes' && <section className={s.group} aria-labelledby="compatible-heading"><h3 id="compatible-heading">Offres partenaires InfiMatch</h3>{data.internal.personalization === 'GENERAL_PROFILE_INCOMPLETE' ? <p className={s.notice}>Ces missions partenaires sont consultables. Votre profil ou votre vérification professionnelle est incomplet : leur compatibilité n’est pas confirmée. <Link to="/profil">Compléter mon profil</Link>.</p> : <p>Des missions InfiMatch qui respectent les critères de votre profil. Les plus pertinentes viennent d’abord ; à pertinence égale, les publications les plus récentes sont prioritaires. Consultez le détail pour comprendre la compatibilité.</p>}
        {data.internal.status === 'UNAVAILABLE' ? <div className={s.notice} role="status"><p>La recherche de missions compatibles est temporairement indisponible.</p><Button variant="outline" onClick={result.reload}>Réessayer les missions compatibles</Button></div> : data.internal.items.length ? cards(data.internal.items, false) : <p className={u.muted}>Aucune mission partenaire disponible dans cette sélection. <Link to="/profil">Vérifiez votre profil</Link>, votre dossier RPPS et vos <Link to="/calendrier">disponibilités</Link>.</p>}
      </section>}
      {origin !== 'partenaires' && <section className={s.group} aria-labelledby="external-heading"><h3 id="external-heading">Offres externes à explorer</h3>
