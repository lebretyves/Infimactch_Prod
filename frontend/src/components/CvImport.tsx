...
export function CvImport({services,existing,onAdd,addBlocked=false}:{addBlocked?:boolean;services:string[];existing:Experience[];onAdd:(values:Experience[])=>void}){
 ...
 <h3>Préremplir mes expériences avec mon CV</h3>{addBlocked&&<p role="status">Vous pouvez analyser votre CV. Enregistrez ou annulez l’expérience en cours de modification avant d’ajouter les expériences proposées.</p>}<p>Importez votre CV : les expériences reconnues seront proposées pour vérification. Le texte est analysé par InfiMatch, sans service tiers ; le fichier n’est pas conservé par cet import. Vos informations personnelles et vos diplômes restent inchangés.</p>
