...
function Metric({label,rate,scope}:{label:string;rate:Rate;scope:string}){return <div style={{padding:16,border:'1px solid var(--line)',borderRadius:12}}><dt style={{fontWeight:650}}>{label}</dt><dd style={{margin:0}}><strong style={{fontSize:24}}>{rate.percent===null?'Non calculable':rate.percent.toLocaleString('fr-FR')+' %'}</strong><p>{rate.numerator} / {rate.denominator}</p><p>{scope}</p>{rate.denominator===0&&<p>Aucun élément dans cette cohorte.</p>}</dd></div>;}
export function EnterpriseConversion({userId}:{userId:string}){
  ...
  return <section aria-labelledby="enterprise-conversion-title" style={{padding:24,background:'white',border:'1px solid var(--line)',borderRadius:16,marginBlock:24}}>
    ...
    {orgs.loading?<p role="status">Chargement des organisations…</p>:orgs.error?<p role="alert">{orgs.error} <Button onClick={orgs.reload}>Réessayer</Button></p>:!orgs.data?.organizations.length?<p>Aucune organisation active disponible.</p>:<>
      <form onSubmit={submit} style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,210px),1fr))',gap:16,alignItems:'end'}}>
      ...
      </form><p>Période maximale : 366 jours. Changer les dates puis calculer ; aucune actualisation automatique.</p>
      {report.loading?<p role="status">Calcul des cohortes…</p>:report.error?<p role="alert">{report.error} <Button onClick={report.reload}>Réessayer</Button></p>:report.data&&<>
        <p><strong>{report.data.organization.name}</strong> · Du {report.data.period.from} au {report.data.period.to}, inclus · État observé le {new Date(report.data.observedAt).toLocaleString('fr-FR',{timeZone:'UTC'})} UTC</p>
        <dl style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,240px),1fr))',gap:16}}>
        ...
        <p><strong>Délai moyen de pourvoi :</strong> {report.data.fillDelay.averageHours===null?'Non calculable':report.data.fillDelay.averageHours.toLocaleString('fr-FR')+' heures'} · {report.data.fillDelay.samples} mission(s) mesurable(s), de la publication à la première affectation encore active ou terminée.</p>
        <p>Sur cette organisation, toutes dates confondues : {report.data.exclusions.demoMissions} mission(s) fictive(s) identifiée(s) exclue(s) ; {report.data.exclusions.undatedPublications} publication(s) et {report.data.exclusions.undatedApplications} candidature(s) sans date initiale attestée, hors cohortes. Ces données manquantes peuvent limiter la représentativité des taux.</p>
        <details><summary>Définitions et limites des indicateurs</summary>{Object.values(report.data.definitions).map(text=><p key={text}>{text}</p>)}<p>Une annulation peut modifier le pourvoi courant sans effacer le fait qu’une candidature a déjà été acceptée. Les statistiques ne mesurent ni les consultations ni le nombre de personnes ayant vu une offre.</p></details>
      </>}
    </>}
  </section>;
}
