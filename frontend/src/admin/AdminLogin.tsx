...
export function AdminLogin({ready}:{ready:(u:User)=>void}){
 ...
 const activate=mode==='invitation'&&passwordSetupRequired===true;
 ...
 async function submit(e:FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   if(!step&&invitationCheck){
    ...
    setEmail(email.trim());setInvitation(invitation.trim());setPasswordSetupRequired(checked.passwordSetupRequired);return;
   }
 ...
 {mode==='invitation'&&<p>{activate?'Choisissez un mot de passe pour votre compte InfiMatch.':'Votre compte InfiMatch possède déjà un mot de passe. Utilisez ce mot de passe pour continuer.'}</p>}
 ...
 {error&&<p role="alert" className="admin-message admin-error">{error}</p>}<Button type="submit" loading={busy} block>{step?.recoveryCodes?'Accéder à l’administration':step?'Vérifier':invitationCheck?'Vérifier mon invitation':mode==='invitation'?'Continuer':'Se connecter'}</Button>
