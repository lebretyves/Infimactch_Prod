...
try {
  const page = await browser.newPage(); const errors=[]; const mutations=[]; let unavailable=false; let sourceEnabled=true;
  ...
  async function visit(path){await page.goto(`${origin}/#${path}`);}
  ...
  await visit('/privacy-requests');await page.getByText('privacy-1',{exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:/supprimer|approuver/i}).count(),0);
