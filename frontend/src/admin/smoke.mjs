...
try {
  ...
  await page.getByRole('button',{name:'Vérifier mon invitation',exact:true}).click();
  await page.getByRole('alert').filter({hasText:'Cet accès est déjà configuré.'}).waitFor();
  assert.equal(await page.getByLabel('Mot de passe',{exact:true}).count(),0,'Invitation never falls back to password login');
  assert.equal(invitationLogins,0);assert.equal(activations,0);
  ...
  assert.equal(activations,1,'Normal login never calls activation');
  assert.equal(invitationLogins,0,'No invitation sent to login');
