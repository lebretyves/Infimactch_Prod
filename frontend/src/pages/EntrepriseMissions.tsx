...
export default function EntrepriseMissions() {
  ...
  return <div className={s.page}>
    ...
    <form className={p.search} onSubmit={event=>{event.preventDefault();search();}}>
      ...
      <div><label htmlFor="mission-shift">Horaires</label><select id="mission-shift" value={draft.shift} onChange={event=>field('shift',event.target.value)}><option value="">Tous les horaires</option><option value="MORNING">Matin</option><option value="AFTERNOON">Après-midi</option><option value="DAY">Jour</option><option value="NIGHT">Nuit</option><option value="MIXED">Jour et nuit</option><option value="UNKNOWN">Non connus</option></select></div>
    ...
    {r.loading?<p role="status">Chargement des missions…</p>:r.error?<div role="alert"><p>{r.error}</p><Button onClick={r.reload}>Réessayer</Button></div>:<>
      ...
      {pages>1&&!outOfRange&&<MissionPagination current={current} count={pages} move={move}/>}
    </>}
