...
export default function Compte() {
  ...
  const remote=useRemote(signal=>api<{request:Closure|null}>('/me/closure-request',{signal}),user?.id||'');
  ...
  async function submit(event:FormEvent) {
    ...
    if(!confirmed||!password){setError('Confirmez les conséquences de votre demande et saisissez votre mot de passe actuel.');return;}
    pending.current=true;setBusy(true);
    try{await api('/me/closure-request',{method:'POST',body:{password}});setPassword('');setConfirmed(false);setMessage('Votre demande a été enregistrée pour examen. Votre compte n’est pas encore clôturé.');remote.reload();}
  ...
  return <div className={s.page}><header className={s.header}><div><h1>Mon compte</h1><p>Consultez et gérez votre demande de clôture.</p></div></header>
    ...
    <section className={s.card}><h2>Clôturer mon compte</h2><p>La demande est examinée par un administrateur. Des candidatures en cours, des missions ouvertes ou affectées, ou votre rôle de dernier responsable d’une organisation peuvent bloquer la clôture.</p><p>Après exécution, l’accès au compte est retiré et les données de profil sont anonymisées. L’historique métier nécessaire peut être conservé : il ne s’agit pas d’un effacement immédiat de toutes les données.</p>
      {remote.loading?<p role="status">Chargement de votre demande…</p>:remote.error?<div role="alert"><p>{remote.error}</p><Button onClick={remote.reload}>Réessayer</Button></div>:<>
        ...
        {!active&&<form onSubmit={submit} noValidate style={{display:'grid',gap:16,marginTop:20}}><PasswordField label="Mot de passe actuel" value={password} onChange={e=>setPassword(e.target.value)} autoComplete="current-password" required disabled={busy}/><label style={{display:'flex',alignItems:'flex-start',gap:10}}><input type="checkbox" style={{width:20,height:20,flexShrink:0,marginTop:3}} checked={confirmed} onChange={e=>setConfirmed(e.target.checked)} disabled={busy}/>Je demande la clôture de mon compte après examen et j’ai compris que mon accès sera retiré, que mon profil sera anonymisé et qu’un historique métier pourra être conservé.</label><Button type="submit" variant="danger" loading={busy}>Confirmer ma demande de clôture</Button></form>}
      </>}
    </section>
  </div>;
}
