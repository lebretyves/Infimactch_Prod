...
export function Accessibilite() {
  return <QualityPage title="Accessibilité : état des travaux" description="Notre objectif est de rendre les parcours InfiMatch utilisables par le plus grand nombre.">
    <section><h2>Évaluation en cours</h2><p>État au 17 septembre 2026. Nous utilisons le RGAA 4.1.2 comme référence. Aucun audit complet n’a établi notre niveau de conformité : aucun pourcentage de conformité n’est annoncé.</p></section>
    ...
    <section><h2>Aides disponibles</h2><p>Un lien « Aller au contenu principal » est disponible au clavier. Le focus reste visible et les animations respectent la préférence de réduction des mouvements. L’application fonctionne sans installation.</p><p>Si un parcours vous bloque, conservez le nom de la page, l’action et le navigateur utilisés, sans mot de passe ni document médical. Le canal public de signalement et l’identité du responsable de publication restent à préciser par le porteur du projet.</p></section>
...
export function Ecoconception() {
  return <QualityPage title="Notre démarche d’écoconception" description="Réduire les ressources utilisées tout en conservant des parcours utiles et accessibles.">
    <section><h2>Actions dans cette version</h2><p>Les écrans sont chargés à la demande pour éviter de transférer tous les formulaires dès l’accueil. Une seule famille de caractères est hébergée sur le site. Les images de sections secondaires utilisent le chargement différé.</p><p>Le mode hors connexion conserve uniquement une courte page d’aide et les icônes. Il ne duplique pas vos documents, votre session ni les réponses de l’API dans un cache persistant.</p></section>
    <section><h2>Mesurer sans surpromettre</h2><p>État au 17 septembre 2026. Le RGESN 2024, qui comporte 78 critères, sert de cadre. Le poids des fichiers produits par le build est suivi avant et après modification. Il ne mesure ni la consommation énergétique réelle ni les Core Web Vitals des utilisateurs.</p><p>Aucun score RGESN, label ou gain de CO₂ n’est revendiqué. Le cycle de vie du matériel, les engagements des hébergeurs et la gouvernance restent à documenter. Les fréquences des imports et automatisations doivent être évaluées séparément côté serveur.</p></section>
