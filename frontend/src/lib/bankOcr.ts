import {completeBankText} from './bankFields';
export async function analyzeBankFile(file:File,signal:AbortSignal,onProgress:(value:string)=>void):Promise<string>{
 ...
 const recognize=async(image:HTMLCanvasElement|File,sparse=false)=>{
  cancelled();if(!worker){onProgress('Chargement du moteur local…');const {createWorker}=await import('tesseract.js');cancelled();worker=await createWorker('fra+eng',1,{workerPath:'/ocr/worker.min.js',corePath:'/ocr/core',langPath:'/ocr/lang',cacheMethod:'none',workerBlobURL:false,logger:()=>{},errorHandler:()=>{}});if(signal.aborted){await worker.terminate();cancelled();}}
  const {PSM}=await import('tesseract.js');cancelled();await worker.setParameters({tessedit_pageseg_mode:sparse?PSM.SPARSE_TEXT:PSM.AUTO});
  onProgress(sparse?'Lecture complémentaire automatique des champs manquants…':'Lecture du document sur cet appareil…');const value=await worker.recognize(image);cancelled();return value.data.text;
 };
 const readFully=async(canvas:HTMLCanvasElement)=>{const first=await recognize(canvas);if(completeBankText(first))return first;cancelled();return first+'\n'+await recognize(canvas,true);};
 try{
  if(file.type!=='application/pdf'){
   const bitmap=await createImageBitmap(file);try{cancelled();if(bitmap.width*bitmap.height>20000000)throw new Error('Image trop grande pour l’analyse locale. Utilisez une image plus petite ou la saisie manuelle.');const scale=Math.min(1,2200/Math.max(bitmap.width,bitmap.height));const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));canvas.getContext('2d')!.drawImage(bitmap,0,0,canvas.width,canvas.height);return await readFully(canvas);}finally{bitmap.close();}
  }
...
export async function createBankCameraReader(signal:AbortSignal,language='fra+eng'){
