...
| --- | --- | --- |
| API InfiMatch | Frontend et administration vers `/api/v1` ; sessions et CSRF ; backend documenté : `https://infimactch-prod-backend.vercel.app` | `frontend/src/services/api.ts`, `annexe/openapi.json` |
...
| n8n | Backend vers `N8N_WEBHOOK_BASE` ; n8n vers `/api/v1/internal/automation/*`, avec authentification de service | `backend/src/automation/automation.module.ts`, `annexe/n8n/` |
...
- `annexe/n8n/InfiMatch-production-*.json` : sept définitions destinées à n8n Cloud. Le configurateur retire les références de credentials lors de l'export ; il faut les associer à nouveau lors d'un import.
- `annexe/n8n/maintenance-quotidienne.json` : maintenance séparée, dont l'activation est confirmée dans la nouvelle lecture du 23 septembre.
- `annexe/n8n/*-test.json` : essais manuels Discord / Teams. `discord-messages.json` est un catalogue, pas un workflow importable. Le relais Discord a son modèle dans `workflows/discord-relay.template.json`.

| Fichier dans annexe/n8n | Fonction / fréquence du JSON |
| --- | --- |
...
| Résumé récent | `annexe/unit-coverage-95.md` | 643/643 tests, 95,07 % des lignes backend |
| Preuves récentes versionnées | `annexe/proofs/unit-coverage-20260923/` | Totaux JSON et journal du retest |
| Rapport HTML généré | `annexe/proofs/unit-coverage/index.html` | Ignoré par Git ; généré par la commande de couverture |
| Totaux générés | `annexe/proofs/unit-coverage/coverage-summary.json` | 11 639 / 12 242 lignes ; branches 89,97 % ; fonctions 82,31 % |
| Preuves historiques | `annexe/proofs/`, notamment `v1-hardening/` | Vérifier la date et le périmètre avant citation |
...
Attention : `annexe/proofs/coverage-totals.json` contient l'ancien résultat 79,38 %, et `annexe/proofs/v1-hardening/result.json` est daté du 17 septembre. Ils ne prouvent pas les 95,07 % récents. Le nouveau seuil de couverture ne constitue pas une nouvelle validation de la production ni des fournisseurs externes.
