...
## France Travail
L'adaptateur et la CLI sont présents. [Catalogue officiel](https://francetravail.io/produits-partages/catalogue/offres-emploi/documentation). Des acquisitions authentifiées ont été documentées depuis ce constat initial ; voir [l'acquisition datée](ACQUISITION_REELLE.md). Les accès et quotas actuels doivent être vérifiés séparément.

L'absence de configuration doit produire un échec explicite. Ne pas créer de manifeste d'acquisition réelle à partir des fixtures.
...
Pour une nouvelle preuve, prévoir un appel borné et contrôler son résultat, sans doubler systématiquement les appels par un dry-run suivi d'un import. Conserver date, périmètre, transformations, rejets et empreinte. Les offres importées sont visibles dans /listings/external et dans la recherche commune. Les filtres stricts excluent les annonces dont les champs requis sont inconnus. Les fixtures SQL de recette utilisent explicitement la source TEST_FIXTURE.
