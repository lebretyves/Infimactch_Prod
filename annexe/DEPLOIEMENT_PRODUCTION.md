...
État documentaire au 24 septembre 2026. [État courant et changements non déployés](ETAT_COURANT.md). [Inventaire des services](DEPLOIEMENT_INVENTAIRE.md) et [migration Supabase](supabase-migration.md).
...
Frontend et API sont des projets Vercel distincts reliés au dépôt production Main. La branche de référence Epitech est `main` ; `Backend` reste une branche historique. L'administration possède un build et une publication séparés : un push applicatif ne prouve pas sa mise à jour.
...
Les derniers commits applicatifs vérifiés et leurs états CI/déploiement sont consignés dans [le registre de livraison](rendu/LIVRAISON_VERIFIEE.json). Le manifeste du rendu fixe le commit remis. Le registre conserve les résultats de sa campagne : ils ne valident pas la branche de revue du 24 septembre. Le budget Actions Epitech était bloqué au dernier constat. Les correctifs documentaires ultérieurs ne prouvent pas une nouvelle recette de production.
