...
export async function processClosures(db:Database,limit=100){
 ...
 const requests=await db.query("SELECT id,account_id FROM closure_request WHERE status='APPROVED' ORDER BY approved_at LIMIT $1",[limit]);
 ...
 for(const request of requests){
  ...
  }catch { failures.push({requestId:request.id,code:"CLOSURE_RETRY_REQUIRED"}); }
 }
