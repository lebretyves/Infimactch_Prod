...
export async function anonymizeAccount(em: SqlClient, accountId: string) {
  ...
  // Draft notes are not signed business records and may contain personal contact details.
  await em.query("DELETE FROM contract_preparation WHERE created_by=$1 OR updated_by=$1 OR assignment_id IN(SELECT id FROM assignment WHERE nurse_id=$1)",[accountId]);
