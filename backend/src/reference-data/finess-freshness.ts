...
export function finessFreshness(snapshot:any,now=new Date()){
 ...
 base.importedAt=Number.isFinite(imported.getTime())?imported.toISOString():null;
 if(!generated.isValid||!current.isValid||generated.toMillis()>current.toMillis())return {...base,status:'INVALID',message:'Date du référentiel FINESS invalide : vérifier la source avant utilisation.'};
