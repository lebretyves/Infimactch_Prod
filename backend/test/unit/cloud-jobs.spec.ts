...
test("cloud dispatch authenticates before processing and bounds work",async()=>{
 ...
 try{const calls:string[]=[];const jobs=new CloudJobsController({dispatch:async(n:number)=>{calls.push("events:"+n);return [];}} as any,{dispatch:async(n:number)=>{calls.push("notifications:"+n);}} as any,{query:async(sql:string,params:unknown[])=>{assert.match(sql,/operational_check/);assert.deepEqual(params,[JSON.stringify({processed:0})]);calls.push("heartbeat");}} as any,{} as any);
