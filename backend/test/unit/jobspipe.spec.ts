...
import { normalizeJobsPipe, fetchJobsPipe } from "../../src/public-data/jobspipe";
import { importOffers } from "../../src/public-data/offers";
const fixture = {id:"jp:123",job_title:"Infirmier IDE",description:"Mission interim dans un service hospitalier.",country_code:"FR",url:"https://jobs.example.org/123",location:"Paris",status:"active"};
test("JobsPipe preserves source, URL and nursing facts",()=>{const o=normalizeJobsPipe(fixture);assert.equal(o.source,"JOBSPIPE");assert.equal(o.sourceId,fixture.id);assert.equal(o.url,fixture.url);assert.equal(o.qualification,"IDE");assert.equal(o.provenance.provider,"JOBSPIPE");});
test("JobsPipe rejects non interim, non nursing, closed and unsafe offers",()=>{for(const change of [{description:"Poste CDI"},{job_title:"Comptable"},{country_code:"US"},{status:"closed"},{expires_at:"2020-01-01"},{url:"javascript:alert(1)"},{url:"https://user:password@example.org"},{description:"Notre cabinet d'intérim accompagne les établissements. Recrutement d'un infirmier."}])assert.throws(()=>normalizeJobsPipe({...fixture,...change}));});
