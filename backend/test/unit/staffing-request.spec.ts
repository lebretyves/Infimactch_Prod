...
test('need period is normalized to exact UTC instants',()=>{const normalized=normalizeNeedDetails(details,Date.parse('2034-01-01T00:00:00Z'));assert.equal(normalized.start,'2035-01-10T05:00:00.000Z');assert.equal(normalized.end,'2035-01-10T13:00:00.000Z');assert.equal(normalized.headcount,2);});
...
test('specialty is required only for specialized blocks',()=>{assert.throws(()=>normalizeNeedDetails({...details,block:'SPECIALIZED'},Date.parse('2034-01-01T00:00:00Z')));assert.throws(()=>normalizeNeedDetails({...details,specialty:'DIGESTIF'},Date.parse('2034-01-01T00:00:00Z')));assert.equal(normalizeNeedDetails({...details,block:'SPECIALIZED',specialty:'DIGESTIF'},Date.parse('2034-01-01T00:00:00Z')).specialty,'DIGESTIF');});
