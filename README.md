...
| Besoin | Document |
| --- | --- |
| Périmètre et critères d'acceptation | [Exigences du projet](docs_intern/REQUIREMENTS_V1.md) |
| Préparer le rendu Epitech | [Livrables et recette finale](docs_intern/rendu/README.md) |
| Trouver un guide technique | [Index documentaire](docs_intern/README.md) |
| Comprendre les services déployés | [Déploiement](docs_intern/DEPLOIEMENT_PRODUCTION.md) |
| Retrouver un ancien document | [Politique d'archivage](docs_intern/ARCHIVES.md) |
...
Pour le développement API avec recompilation : `npm run dev`. Le worker utilise `npm run worker` après compilation ; son exécution peut traiter des événements. Voir le [guide frontend](frontend/README.md) et la [configuration des services](docs_intern/quality/CONFIGURATION.md).
...
Le [dossier de rendu](docs_intern/rendu/README.md) distingue livrables disponibles et validations ouvertes. Aucune conformité RGAA complète ni économie financière/carbone n'est revendiquée.

## API et exploitation

Les routes métier utilisent `/api/v1`. Les écritures authentifiées nécessitent les protections de session, d'origine et CSRF prévues par le client. Les droits sont contrôlés côté serveur. L'[export OpenAPI](docs_intern/openapi.json) complète la documentation servie par l'application.

Les variables `VITE_*` sont publiques : ne jamais y mettre de secret. L'administration, le site et l'API ont des livraisons distinctes. Voir [le déploiement](docs_intern/DEPLOIEMENT_PRODUCTION.md), [les sauvegardes](docs_intern/quality/SAUVEGARDES_OPERATIONNELLES_2026-09-19.md) et [les automatisations](docs_intern/rendu/README.md#automatisations).
...
[Seconde vérification documentaire et réserves restantes](docs_intern/VERIFICATION_DOCUMENTAIRE_2026-09-21.md).

La [revue du code du 21 septembre](docs_intern/quality/REVUE_CODE_2026-09-21.md) décrit le nettoyage, les corrections et les limites des vérifications.
