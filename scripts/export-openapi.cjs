...
(async()=>{
 ...
 try {
  ...
  const output=resolve(__dirname,'../docs_intern/openapi.json');
  ...
  console.log(JSON.stringify({exported:'docs_intern/openapi.json',paths:Object.keys(doc.paths).length,operations,webhookBearer:true,bicOptional:true}));
