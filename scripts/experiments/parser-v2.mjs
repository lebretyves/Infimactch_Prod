...
export function parseOfferV2(row){
 const result={id:row.id,source:row.source,title:row.title,url:row.url,fields:[],alerts:[],unclassified:[]};
 const add=(field,value,evidence,origin='DESCRIPTION',state='MENTION_A_CONFIRMER')=>{if(!result.fields.some(f=>f.field===field&&JSON.stringify(f.value)===JSON.stringify(value)&&f.evidence===evidence))result.fields.push({field,value,evidence,origin,state});};
 ...
 for(const {text,origin} of texts){const s=fold(text);const before=result.fields.length;
  ...
  for(const [field,terms] of Object.entries(dictionaries))for(const [value,re] of Object.entries(terms)){const m=s.match(re);if(m){
   ...
   const prefix=s.slice(Math.max(0,m.index-28),m.index);
  ...
  if((/contrat|poste|recrut|mission/.test(s)||/^(cdi|cdd)\b/.test(s))&&!/en parallele|selon vos disponibilites|si vous le souhaitez/.test(s)){
   for(const [v,re] of [['CDI',/\bcdi\b|duree indeterminee/],['CDD',/\bcdd\b|duree determinee/],['INTERIM',/interim/],['VACATION',/vacation/]])if(re.test(s))add('contrat_texte',v,text,origin,'EXPLICITE_A_VERIFIER');
  }
  if(/experience/.test(s)){
   const m=s.match(/(\d+|une?|deux|trois|cinq)\s*(?:ans?|annees?|mois)\b/);
   if(m){const n={un:1,une:1,deux:2,trois:3,cinq:5}[m[1]]??Number(m[1]);add('experience_duree',{amount:n,unit:/mois/.test(m[0])?'MOIS':'ANS'},text,origin,/minimum|minimale|au moins/.test(s)?'EXIGENCE_TEXTE':state);}
  ...
  for(const [v,re] of [['REPAS',/repas|titres? restaurants?/],['TRANSPORT',/navigo|transport|carte essence|vl de fonction/],['PRIMES',/prime|segur|interessement/],['MUTUELLE',/mutuelle/],['LOGEMENT',/logement|hebergement/],['RTT',/\brtt\b/]])if(re.test(s))add('avantage',v,text,origin,'A_STRUCTURER');
 ...
 const exps=result.fields.filter(f=>f.field==='experience_duree'&&f.value.unit==='ANS');const apiExp=Number(facts?.experience?.label?.match(/^\d+/)?.[0]);
 if(exps.some(f=>apiExp&&f.value.amount!==apiExp))result.alerts.push('EXPERIENCE_API_TEXTE_DIVERGENTE');
 if(result.fields.some(f=>f.field==='contrat_texte'&&['CDI','CDD','VACATION'].includes(f.value))&&facts?.contract?.code==='MIS')result.alerts.push('CONTRAT_IMPORT_TEXTE_DIVERGENT');
 if(result.fields.some(f=>f.field==='qualification_titre'&&f.value==='IBODE')&&/de infirmier de bloc serait un plus/.test(fold(row.description)))result.alerts.push('IBODE_TITRE_MAIS_DIPLOME_SOUHAITE_DANS_TEXTE');
 if(result.fields.some(f=>f.field==='horaire_type'&&f.value==='NUIT')&&/journee/.test(fold(facts?.workingTime)))result.alerts.push('HORAIRES_API_TEXTE_A_RECONCILIER');
