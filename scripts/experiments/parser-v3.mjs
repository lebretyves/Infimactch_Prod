...
export function parseOfferV3(row){
 ...
 for(const {s:evidence,origin} of clauses){const s=fold(evidence);
  ...
  for(const [field,values] of Object.entries(terms))for(const [value,pattern] of Object.entries(values)){
   ...
   const prefix=s.slice(Math.max(0,m.index-30),m.index);
  ...
  if(/indemnite|conges payes|prime|rembours|mutuelle|repas|navigo|titre.{0,3}restaurant/.test(s)){
   ...
   for(const m of s.matchAll(/(\d+(?:[.,]\d+)?)\s*(?:euros?|€)(?:\s*par\s+(jour(?: travaille)?|repas))?/g)){
    ...
    const nearby=s.slice(Math.max(0,m.index-55),m.index+m[0].length+35);
 ...
 const times=new Set(r.fields.filter(f=>f.field==='temps_travail').map(f=>f.value));
 ...
 r.reviewQueue=clauses.filter(c=>important.test(fold(c.s))&&!/cnil|donnees personnelles|\bde ca\b/.test(fold(c.s))).map(c=>({evidence:c.s,extractedFields:[...new Set(r.fields.filter(f=>f.evidence===c.s).map(f=>f.field))],status:'RELECTURE_PARTIELLE_REQUISE'}));
