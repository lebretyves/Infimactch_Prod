...
export function structuredDetails(text){
 const s=text.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
 const result={salaryRanges:[],shiftDurationsHours:[],hourPairs:[]};
 ...
 for(const m of s.matchAll(re)){
  const before=s.slice(Math.max(0,m.index-65),m.index);
  ...
  const number=(v,k)=>Number(v.replace(/[ \u00a0]/g,'').replace(',','.'))*(k?1000:1);
