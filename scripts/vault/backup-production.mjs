...
try{await withRole('operator',async token=>{
 ...
 const keyMetadata=newBackupKeyMetadata(values),key=backupRoot(values,keyMetadata);
 ...
 await writeFile(resolve(folder,'manifest.json'),JSON.stringify({...keyMetadata,date:new Date().toISOString(),status:'COMPLETE',algorithm:'AES-256-GCM',derivation:'HKDF-SHA256 / infimatch-production-backup-v1 / BACKUP_KEY (versioned)',files,restoreNote:'Replay the CURRENT Atlas erasureledger before reopening a SQL restore. Never replace that ledger with an older snapshot.'},null,2)+'\n');
