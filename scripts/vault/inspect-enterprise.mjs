...
await withRole('operator',async token=>{const v=(await request('kv/data/infimatch/v1/production',{token})).data.data;const db=new Client({connectionString:v.DATABASE_URL});await db.connect();try{
