...
EXCLUDE={'annexe/INVENTAIRE_DOCUMENTAIRE.csv','annexe/INVENTAIRE_DOCUMENTAIRE.md'}
def category(name):
...
for name in sorted(set(names)):
 p=ROOT/name
 if not name.startswith('annexe/') or name in EXCLUDE or not p.is_file() or p.name.startswith('~$') or '__pycache__' in p.parts:continue
...
(ROOT/'annexe/INVENTAIRE_DOCUMENTAIRE.csv').write_text(out.getvalue(),encoding='utf8')
...
(ROOT/'annexe/INVENTAIRE_DOCUMENTAIRE.md').write_text(body,encoding='utf8')
